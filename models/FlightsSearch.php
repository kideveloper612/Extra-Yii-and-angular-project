...
class FlightsSearch extends Model
{
    ...
    const SINGLE_CITY_ROUTE = 'single';
    const MULTI_CITY_ROUTE = 'multi';
    ...
    public function rules()
    {
        ...
        return [
            [
                ...
                'required', 'on' => self::SINGLE_CITY_ROUTE 
            ],
            [
                [
                    'adults',
            ...
            [['segments'], 'required', 'on' => self::MULTI_CITY_ROUTE ]
        ];
    ...
    public function search_multicity()
    {
        $api = new TravelPayoutsApi($this->token);
        $api->setHost($_SERVER['HTTP_HOST'])
            ->setMarker($this->marker)
        ...
        foreach($this->segments as $segment){
            $api->addSegment($segment['origin'], $segment['destination'], $segment['date']);
        }
